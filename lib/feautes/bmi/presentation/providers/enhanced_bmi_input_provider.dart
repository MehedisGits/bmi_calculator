...
class EnhancedBMIInputNotifier extends Notifier<BMIInputState> {
...
final enhancedBMIInputProvider = NotifierProvider<EnhancedBMIInputNotifier, BMIInputState>(
  EnhancedBMIInputNotifier.new,
);

/// Convenience provider for just the input data
final bmiInputDataProvider = Provider<BMIInput>((ref) {
  return ref.watch(enhancedBMIInputProvider).input;
});

/// Provider for input validation status
final bmiInputValidationProvider = Provider<bool>((ref) {
  return ref.watch(enhancedBMIInputProvider).isValid;
});

/// Provider for input error messages
final bmiInputErrorProvider = Provider<String?>((ref) {
  return ref.watch(enhancedBMIInputProvider).error?.message;
});

/// Extension for easier access to enhanced provider
extension EnhancedBMIInputExtension on WidgetRef {
  EnhancedBMIInputNotifier get bmiInput => read(enhancedBMIInputProvider.notifier);
  BMIInputState get bmiInputState => read(enhancedBMIInputProvider);
}
